...
// ========================
// Shows upgrade prompt for free users and beta expiry countdown.
// Paid founders see nothing.
...
export function FreeTierBanner() {
      ...
      >
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">
            {betaDaysRemaining > 0
              ? `${betaDaysRemaining} days remaining. Upgrade to Founder before expiry to keep access.`
              : 'Your beta access has expired. Upgrade to continue.'}
          </p>
        </div>
        <button
  ...
  // Standard free user banner — upgrade to Founder
  return (
    <div
      className="flex items-center justify-between px-6 py-4 rounded-xl mb-6"
      style={{
    ...
    >
      <div>
        <p className="text-sm font-semibold text-[#09090B]">
          Upgrade to Founder — ₹3,999/year
        </p>
        <p className="text-sm text-zinc-500 mt-0.5">
          Unlock AI coaching, check-ins, board reports, and full dashboard access.
        </p>
      </div>
      <button
        onClick={() => navigate('/checkout?plan=founder')}
