// HeroSection - Gradient corrected: pure black top → teal mid → bright cyan bottom
// Matches reference images: text in dark upper zone, card floating over cyan base
...
const FloatingOrb = ({ style, duration = 8, delay = 0 }) => (
  <motion.div
    ...
    style={style}
    animate={{ y: [0, -16, 0], scale: [1, 1.06, 1], opacity: [style.opacity ?? 1, (style.opacity ?? 1) * 0.70, style.opacity ?? 1] }}
...
export const HeroSection = () => {
  ...
  return (
    ...
    >
      {/* ═══ GRADIENT: pure black top → cyan bottom (matches reference exactly) ═══ */}

      {/* 1. Hard black base */}
      <div className="absolute inset-0" style={{ background: '#000' }} />

      {/* 2. Main vertical gradient — the defining layer */}
      ...
      {/* 3. Bottom radial bloom — bright cyan center glow at base (mouse-reactive) */}
      <motion.div
      ...
      {/* 4. Left + right side dark vignettes */}
      <div className="absolute inset-0 pointer-events-none" style={{
        background: `
      ...
      {/* 5. Top black clamp — ensures top stays absolutely black */}
      <div className="absolute inset-x-0 top-0 pointer-events-none" style={{
        height: '40%',
        background: 'linear-gradient(to bottom, #000 0%, #000 35%, transparent 100%)',
      }} />

      {/* 6. Dot grid — only visible in lower cyan zone */}
      <motion.div
        className="absolute inset-0 pointer-events-none"
        style={{
          y: bgY3,
      ...
      >
        ...
        <motion.h1
          ...
          variants={headlineVariants} initial="hidden" animate="visible"
        ...
        >
          {/* Primary — white solid (matches ref "Start building" button) */}
          <motion.button
        ...
        {/* ── Dashboard / chart card — floats partially off bottom ── */}
        <motion.div
        ...
        >
          ...
          >
            {/* Glass card — white/light (matches reference UI screenshots) */}
            ...
            <div className="relative z-10">
              {/* macOS chrome */}
              <div className="flex items-center h-10 px-4 border-b border-[rgba(0,0,0,0.06)]"
              ...
              <div className="p-6 bg-white">
                ...
                {/* Chart — light background, teal line, grid lines like reference */}
                <div className="h-[240px] md:h-[280px] -mx-1 relative">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData} margin={{ top: 12, right: 20, left: 0, bottom: 12 }}>
                      ...
                      {/* Grid — horizontal only, very subtle, matches reference dashboard */}
                      <CartesianGrid
                        strokeDasharray="3 3"
                      ...
                      <YAxis
                        ...
                        domain={[0, 120000000]} width={32}
                      />
                      <ReferenceLine
                        y={100000000}
                        stroke={C.midCyan} strokeDasharray="5 5" strokeOpacity={0.30}
                      />
                      <Area
                        type="monotone" dataKey="arr"
                        stroke={C.midCyan} strokeWidth={2.5}
